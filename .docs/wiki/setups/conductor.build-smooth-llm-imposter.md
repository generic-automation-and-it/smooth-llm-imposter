...
umask 077
{
  ...
  for var in $(compgen -e | grep -E '^(Imposter__|Admin__|ConnectionStrings__|[A-Z0-9]+_(API_KEY|AUTH_SCHEME|BASE_URL|DIALECT|IS_DEFAULT|OPENAI_UPSTREAM_API|REQUEST_NORMALIZATION|ANTHROPIC_VERSION))$)'); do
    printf '%s=%s\n' "$var" "${!var}"
  done
} > "$ENV_FILE"
chmod 600 "$ENV_FILE"
