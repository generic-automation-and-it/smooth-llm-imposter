...
> and keys are `<NAME>_API_KEY` / `<NAME>_AUTH_TOKEN` (conventional) or `Imposter__Providers__<name>__Secret`
...
>
> **`_API_KEY` vs `_AUTH_TOKEN` follows the provider's auth scheme.** Both fill the same provider `Secret`, but
> which one wins is scheme-driven: a **`Bearer`** provider prefers `<NAME>_AUTH_TOKEN`, an **`ApiKey`** provider
> prefers `<NAME>_API_KEY` (the off-scheme var stays a fallback so a single var still authenticates). The shipped
> LEGO-gateway imposters set the scheme explicitly — `anthropic` is `Bearer` (use `ANTHROPIC_AUTH_TOKEN`), `openai`
> is `ApiKey` (use `OPENAI_API_KEY`) — so a personal `ANTHROPIC_API_KEY` is never sent as a Bearer token to the
> gateway.
...
defaults by dialect (openai → Bearer, anthropic → ApiKey); the shipped providers set it explicitly, and the
matching secret var follows that scheme (`ANTHROPIC_AUTH_TOKEN` for the Bearer `anthropic` imposter,
`OPENAI_API_KEY` for the ApiKey `openai` imposter):

```bash
export ANTHROPIC_AUTH_TOKEN="sk-your-lego-token"      # Bearer `anthropic` imposter (LEGO /claude)
export OPENAI_API_KEY="sk-your-lego-token"            # ApiKey `openai` imposter (LEGO /openai)
export OPENCODE_GO_API_KEY="sk-your-opencode-key"
export OPENROUTER_API_KEY="sk-your-openrouter-key"
```

> Pass `-e ANTHROPIC_API_KEY` instead only if you flip the `anthropic` provider to `AuthScheme=ApiKey`
> (`-e ANTHROPIC_AUTH_SCHEME=ApiKey`). With the shipped `Bearer` scheme, `ANTHROPIC_AUTH_TOKEN` is preferred and a
> stray `ANTHROPIC_API_KEY` is ignored while the token is present.
