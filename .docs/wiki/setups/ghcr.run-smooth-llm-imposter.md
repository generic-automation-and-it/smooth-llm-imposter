...
> providers set the scheme explicitly (e.g. the `opencode-go-*` and `openrouter-*` siblings). If a provider's
...
matching secret var follows that scheme:
...
> then follows it (a `Bearer` provider prefers `<NAME>_AUTH_TOKEN`, an `ApiKey` provider prefers `<NAME>_API_KEY`,
> and the off-scheme var stays a fallback). If a provider's gateway wants the credential in a non-standard header
> name, set `-e Imposter__Providers__<name>__AuthHeader=api-key` to relocate it.
