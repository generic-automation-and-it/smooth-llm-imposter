...
|------|----------|--------|
| [LADR-01](./ladrs/LADR-01-dictionary-keyed-providers.md) | Key providers by name (Dictionary), hard cutover; `Name` optional override | Draft |
| [LADR-02](./ladrs/LADR-02-conventional-env-surface.md) | Conventional `<NAME>_<FIELD>` env surface, case-insensitive, full field set | Draft |
| [LADR-03](./ladrs/LADR-03-resolution-mechanism.md) | Post-configure resolver, key→prefix normalization, precedence, legacy-shape guard | Draft |
...
|-----|-----------|------------------|--------|
| [NFR-01](./nfrs/NFR-01-override-stability.md) | Override Stability | Override targets same provider under any declaration order; no positional addressing | Draft |
| [NFR-02](./nfrs/NFR-02-migration-safety.md) | Migration Safety | Legacy array / numeric / case-dup keys fail fast at startup with guidance | Draft |
| [NFR-03](./nfrs/NFR-03-secret-confidentiality.md) | Security | Resolved secret values never logged or surfaced in errors | Draft |
| [NFR-04](./nfrs/NFR-04-statelessness.md) | Statelessness | Resolution is config/env only — zero DB connections, zero upstream calls | Draft |
