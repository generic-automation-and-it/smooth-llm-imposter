...
| Simple messages are broadly compatible: Chat `messages[]` can become Responses `input`; inverse can become Chat `messages[]`. | Covered. `input`/`instructions` convert to `messages`; a `message` Item whose content does not survive downgrade (null, empty, or built only from unsupported content parts) is **dropped** rather than emitted as a message with neither content nor `tool_calls`. | None. Strict Chat upstreams (Moonshot) 400 on empty messages (`message at position N with role 'assistant' must not be empty`); dropping the empty turn keeps the transcript valid without fabricating placeholder content (LADR-02). |
...
| Conversation state differs: Chat callers replay `messages`; Responses can replay `output`, use `previous_response_id`, or use the Conversations API (`conversation`). | Manual replay via `input` is partially covered. Neither `previous_response_id` nor `conversation` can be resolved by a stateless proxy. | Reject **both** `previous_response_id` and `conversation` on the downgrade path (400, present-and-non-null guarded); clients must replay the needed Items in `input` for Chat-compatible routing. `store` is forwarded unconditionally — it is a valid Chat Completions parameter, not a Responses-only state pointer, so passthrough is safe and a per-provider capability gate is unwarranted (revisit only if a strict upstream is observed to 400 on it). |
| Generation controls: Responses carries reasoning depth as `reasoning.effort`; Chat uses top-level `reasoning_effort`. Other knobs (`stop`, `metadata`, `logit_bias`, `logprobs`, `top_logprobs`) share their shape across both APIs. | Previously dropped by allowlist fallthrough — a silent behavior change (e.g. dropped `stop` alters generation; dropped effort reasons at default depth). | Convert compatible `reasoning.effort` (`minimal`/`low`/`medium`/`high`) → `reasoning_effort`; drop `none`/unknown (`none` disables tool calling on GPT-5.4+ Chat, and this path is tool-heavy). Pass the shared knobs through as named allowlist additions. `n` is N/A (Responses has no `n`). Never a blanket copy — Responses-only fields would 400 a Chat upstream. |
