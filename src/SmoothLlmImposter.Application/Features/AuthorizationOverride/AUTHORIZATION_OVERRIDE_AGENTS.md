...
Runtime, in-memory, per-dialect switch (HLD 003) that forces the **passthrough** path to present the active
stored credential as `Authorization: Bearer`, dropping `x-api-key`. Default OFF, never persisted, OFF on
restart. The forwarding behaviour it gates lives in `Features/Routing/` (`ImposterRouter`, `UpstreamForwarder`).
...
- **Never persist the switch.** It is an in-memory singleton (`ConcurrentDictionary<ApiDialect,bool>`). No
...
- **Fail closed.** Arm-time: `Set` returns `NoActiveCredential` (→ endpoint 403) when no active credential
  exists; the switch stays OFF. Request-time fail-closed (403 dialect-shaped) lives in `ImposterRouter`, not
  here (LADR-005). Never fall back to `x-api-key`.
...
- **Admin-only control surface.** The `PUT`/`DELETE`/`GET /routing/{dialect}/override-authorization` endpoints
  require `AdminPolicy`; the proxy `/v1/*` endpoints stay key-less (LADR-004 / NFR-002).

## Key Behaviors

- `Set` distinguishes `Armed` vs `NoActiveCredential` via `SetAuthorizationOverrideResult`; the Host maps the
  latter to a `403` admin-shaped problem detail. `Clear` is idempotent. `Get` reports state.
...
- The switch is registered **Singleton**; a fresh container reads OFF for every dialect.
