...
{
    public sealed record Request(string Dialect, string? ProviderName, string? Actor) : IRequest<AuthorizationOverrideState>;
        ...
        ILogger<Handler> logger) : IRequestHandler<Request, AuthorizationOverrideState>
    {
        public ValueTask<AuthorizationOverrideState> Handle(Request request, CancellationToken cancellationToken)
        {
            ...
            overrideSwitch.Disable(dialect, providerName);

            // NFR-003: one secret-free Information audit line per toggle (actor + dialect + action).
            logger.LogInformation(
                "Passthrough authorization override disabled by {Actor} for {Dialect}/{ProviderName}",
                request.Actor ?? "unknown",
                token,
                providerName);

            return ValueTask.FromResult(new AuthorizationOverrideState(token, providerName, Enabled: false));
        }
    }
}
