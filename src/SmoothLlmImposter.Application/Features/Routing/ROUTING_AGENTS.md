...
- **`imposter-upstream` uses targeted forwarder retries, not the standard resilience stack.** SSE streams outlive
  standard timeouts, so the named client keeps an infinite timeout bounded by the caller's `CancellationToken`.
  `UpstreamForwarder` rebuilds the request per attempt and retries only pre-response outbound transport failures
  (`HttpRequestException`) and response-header timeouts. It uses fixed 1s, 2s, and 5s delays with progressively
  longer header timeouts (60s, 120s, 300s, 600s). It does not retry upstream 5xx responses from LLM POSTs,
  because those may already be processed and billable (see HLD LADR-003).
