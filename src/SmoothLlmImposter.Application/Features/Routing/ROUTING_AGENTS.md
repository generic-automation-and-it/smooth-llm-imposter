...
- **`AuthHeader` overrides the header *name* only; the *value* still follows `AuthScheme`.** A gateway that
  wants the credential in a non-standard header (the LEGO codex gateway expects `api-key`, not the ApiKey
  ...
  Conventional env override: `<PREFIX>_AUTH_HEADER`.
