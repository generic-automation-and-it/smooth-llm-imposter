...
- **Keep upstream streaming transport infinite at the `HttpClient` layer.** Time-bound outbound LLM calls in
  `UpstreamForwarder` with per-attempt header timeouts so `ResponseHeadersRead` retries/timeouts apply only
...
`Microsoft.EntityFrameworkCore(.Relational/.Design/.Tools)`, `Npgsql.EntityFrameworkCore.PostgreSQL`, `Refit.HttpClientFactory` — declared centrally in `Directory.Packages.props`.
