...
- 2026-07-29: Conductor build setup removes a redundant `OpenAiUpstreamApi=chat_completions` override from the `opencode-go-openai` workspace — the image's `appsettings.json` already ships that value, so GPT-route behavior (`/v1/chat/completions`) is unchanged. This is a redundant-override cleanup, not an API switch; matching the "uses the image default" framing in `.docs/wiki/setups/conductor.build-smooth-llm-imposter.md`.
