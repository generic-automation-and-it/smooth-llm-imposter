...
- 2026-07-25: HLD 010 switch family — `--who?` (routing probe with session info) and `--newsession` (mint synthetic session id + store caller→synthetic mapping) implemented; `ISessionTranslationDictionary` singleton + forward-path translation seam; diagnostic logging added to `WhoMessageResponder` (non-match reasons) and `RoutingEndpoints` (feature-disabled, translation applied). **Breaking change:** Bare `who?` trigger from the initial HLD 010 implementation (see the immediately following 2026-07-25 entry) was replaced by `--who?`. LADRs/NFRs → Accepted; HLD → Completed.
- 2026-07-25: HLD 010 who-message introspection — last user message `--who?` (exact match, trimmed, case-sensitive, non-streaming) short-circuits the forward path with a dialect-shaped synthetic reply naming the inbound model, the resolved upstream target (or `passthrough`), the auth scheme, and session identity; gated on `Imposter:WhoMessage:Enabled` (default `true`, env `IMPOSTER_WHO_MESSAGE_ENABLED`). [NOTE: trigger was `who?` initially, subsequently renamed to `--who?` — see the related switch-family entry directly above.]
