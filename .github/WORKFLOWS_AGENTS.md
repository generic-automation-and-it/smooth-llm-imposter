...
| Date | Change | Ref |
|:-----|:-------|:----|
| 2026-07-30 | Dockerfile base images switched from Debian to `-alpine` .NET 10 variants (`sdk:10.0-alpine`, `aspnet:10.0-alpine`). Verified multi-platform build still succeeds; image ~131 MB (was ~240 MB). Alpine ships `wget` only (no `curl`). | |
