...
- **Proxy CA trust (defensive, currently a no-op).** `imposter-container.sh` extracts the image's own CA
  bundle via `docker run --entrypoint cat`, appends
  `/etc/pki/ca-trust/source/anchors/vercel-proxy-ca.pem` to it when that host path exists, and bind-mounts
  the merged bundle over `/etc/ssl/certs/ca-certificates.crt` inside the container. A container's isolated
  filesystem does not otherwise inherit the Vercel Sandbox proxy CA (see
  [runtimes docs](https://vercel.com/docs/sandbox/concepts/runtimes#proxy-ca-certificates)). Under this
  workspace's default `allow-all` network policy the sandbox firewall never terminates TLS, so nothing
  changes in practice today — the merge exists only so a future restrictive network policy
  (`transform`/`forwardURL` rules) doesn't silently break outbound HTTPS from the container's calls to
  `opencode.ai`/`openrouter.ai`. Skipped, never fatal, on any host without that cert (local macOS, non-Vercel
  environments) or if the image's bundle can't be read. Regenerated every run at
  `/tmp/smooth-llm-imposter-ca-bundle.crt`, never left stale, because the container is always recreated from
...
| 2026-09-20 | Snapshot script: bumped the OpenCode CLI installer from `https://opencode.ai/install` to `https://opencode.ai/v2/install`. | — |
| 2026-09-20 | Fixed a real build failure: step [4] (Configuring RTK) invoked `sudo rtk ...`, but rtk's installer places the binary under `$HOME/.local/bin`, which is not on sudo's `secure_path` — `sudo` failed instantly with `sudo: rtk: command not found`, and the `expect` block wrapping the first call then failed with a misleading `spawn id ... not open` once its spawned process had already exited, aborting the whole snapshot with exit code 1 (reproduced live on a `smooth-ai-report-review` workspace build). Fixed by resolving `RTK_BIN="$(command -v rtk)"` once outside sudo (the current shell's `PATH` already covers `~/.local/bin` per step [3]) and invoking that absolute path under `sudo` instead of the bare name. Also caught and fixed a self-inflicted bug while writing that fix: an apostrophe inside a `${VAR:?message}` parameter-expansion error string breaks bash's parser even inside enclosing double quotes — a real, if obscure, quoting gotcha, not a typo — verified by bisecting a minimal repro down to that single construct; reworded the message to avoid the apostrophe. Both the snapshot-script prose and every embedded bash block in the wiki doc re-verified with `bash -n` after the fix. Separately noted, not fixed (informational only): the transcript also showed `WARNING: Multiple managed Codex installs can be ambiguous because PATH order decides which one runs` — Conductor's own managed Codex CLI at `/conductor/bin/codex` coexists with step [2]'s `curl .../codex/install.sh` standalone install; non-fatal, left as-is. | — |
| 2026-09-20 | Two defensive, currently-no-op fixes prompted by re-reading Vercel's Sandbox runtimes docs. (1) The snapshot script now fails fast with a clear message if `dnf` is absent, instead of failing confusingly mid-`apt`-less step [1] — Vercel's docs mark the Amazon-Linux-2023-based legacy runtime this whole snapshot script depends on as deprecated in favor of an apt-based Ubuntu managed image as the new sandbox default; verified this workspace is still AL2023 today. (2) `imposter-container.sh` now extracts the image's CA bundle, appends the Vercel Sandbox proxy CA when present on the host, and bind-mounts the merged bundle over the container's trust store — containers don't otherwise inherit that CA, and while the workspace's default `allow-all` network policy never triggers TLS termination today (verified against Vercel's firewall docs — only `transform`/`forwardURL` rules terminate TLS), a future restrictive policy would otherwise silently break the container's outbound HTTPS to `opencode.ai`/`openrouter.ai`. Both changes are no-ops under current conditions (dnf present, no host proxy CA reachable behavior change) — verified the CA-merge logic's three branches (present/absent host CA, failed image-bundle read) in isolation with a stubbed `docker` wrapper, since this sandbox has no Docker daemon to run the real script end-to-end. Wiki paste-in copy updated in lockstep. | — |
